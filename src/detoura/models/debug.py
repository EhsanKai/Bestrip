...
class BeamRound(BaseModel):
    ...
    # --- V5.2.1: the other things a round can buy ----------------------
    frontier_size: int = 0
    """Itineraries on the Pareto frontier after this round."""
    new_city_sets: int = 0
    """City combinations this round found that no narrower round had."""
    competitive_found: int = 0
    """New itineraries within striking distance of the best (see COMPETITIVE_MARGIN)."""
    stop_reason: str = ""
    """Why the ladder ended here: empty while it continued."""

    @property
    def productive(self) -> bool:
        """Did this round buy anything at all, on any axis?"""
        return (
            self.improvement > 0
            or self.new_city_sets > 0
            or self.competitive_found > 0
        )
...
class SearchDebug(BaseModel):
    ...
    Deliberately free of timing. A :class:`BeamRound` records what a round
    *found*, which is deterministic and comparable between runs; how long it
    took is a measurement of this machine on this day. Mixing the two would
    make two identical searches compare unequal, which is exactly the
    determinism guarantee this project asserts in its tests.
    """
    beam_round_seconds: list[float] = Field(default_factory=list)
    """Cumulative wall clock at the end of each round (V5.2.1). Not deterministic."""
