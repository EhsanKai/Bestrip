...
class StaySummary(BaseModel):
    ...
    """Where this room rate came from and when (V5.3).

    Carried up from the chosen :class:`AccommodationOption` so that an
    itinerary's freshness is the worst of *all* its prices, not just its
    flights. ``None`` from a provider that does not stamp its offers, which
...
class BaselineResult(BaseModel):
    ...
    nights: int = 0
    usable_destination_minutes: int = 0
    """Sightseeing minutes the baseline actually buys (V5).

    The baseline comparison is a signature Detoura component and its whole
    point is that price is not the only axis. Comparing our usable hours
    against a baseline that reports zero would make every trip look infinitely
    better, which is the opposite of the honest comparison the brand promises.
    """
